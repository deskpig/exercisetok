...
const base: Evaluation['ratings'] = { depression: true, exercise: true, relationship: true, dose: 'absent', intensity: 'absent', duration: 'absent', indication: 'absent', safety: 'absent', conflict: false, false_claim: false, actionable: false };
...
describe('study classification', () => {
  ...
  it('excludes failed inclusion even with false claims', () => expect(result({ depression: false, false_claim: true })).toMatchObject({ eligibility: 'excluded', suggested: null }));
  ...
  it('allows completed exclusion with only screening answered', () => expect(answerErrors(sampleRubric, { depression:false, exercise:true, relationship:false })).toEqual([]));
  ...
  it('uses uploaded references and threshold, not study-specific IDs', () => {
    const rubric = { ...sampleRubric, classification: { inclusionFields:['exercise'], domainFields:['dose'], conflictFields:['conflict'], actionableField:'actionable', accurateThreshold:1 } };
    expect(classify(rubric, { exercise:true, dose:'accurate', conflict:false, actionable:false }).suggested).toBe('congruent');
  });
});
